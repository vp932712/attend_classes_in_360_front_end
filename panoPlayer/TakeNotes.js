...
class TakeNotes extends Component {
  ...
  handleDelete() {
    ...
    if (this.state.displayArray[0][0] !== '|') {
      ...
      this.setState({displayArray: arr, start: start, end: end});
  ...
  handleSubmit() {
    ...
    for (let i = 0; i < submitArray.length; i++) {
      submitArray[i] = submitArray[i].split("").filter((element) => {
        return element !== '|'
      }).join("");
    }
    this.setState({
  ...
  showScroll() {
    ...
    for (var i = 0; i < this.state.displayArray.length; i++) {
      if (this.state.displayArray[i] != '')
        total++;
      }

    return total > this.state.rows
      ? 1
      : this.state.opacity;
  }
